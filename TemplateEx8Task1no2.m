...
%Given accuracy for the observations
s_dist = 0.1;             %[m]
...
%Iteration 1
while abs(s_o_dist-1)>DELTA && abs(s_o_dir-1)>DELTA
    
    %VC Matrix of the observations
    C_ll_dist = a_s^2*s_dist^2*ones(5,1);
    C_ll_dir = a_r^2*s_dir^2*ones(9,1);
    C_ll = diag([C_ll_dist;C_ll_dir]);
    
    %Cofactor matrix of the observations
    Q_ll=1/sigma_0^2*C_ll;
...
%Update new a posteriori factors for each group of observations 
a_s = a_s*s_o_dist;
a_r = a_r*s_o_dir; 
...
%Final a posteriori standard deviations factors 
